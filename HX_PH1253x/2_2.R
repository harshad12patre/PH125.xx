#The ACT is a standardized college admissions test used in the United States. The four multi-part questions in this assessment all involve simulating some ACT test scores and answering probability questions about them.For the three year period 2016-2018, ACT standardized test scores were approximately normally distributed with a mean of 20.9 and standard deviation of 5.7. (Real ACT scores are integers between 1 and 36, but we will ignore this detail and use continuous values instead.)First we'll simulate an ACT test score dataset and answer some questions about it.Set the seed to 16, then use rnorm() to generate a normal distribution of 10000 tests with a mean of 20.9 and standard deviation of 5.7. Save these values as act_scores. You'll be using this dataset throughout these four multi-part questions.
set.seed(16)
...
#2.# Set x equal to the sequence of integers 1 to 36. Use dnorm to determine the value
...
#3a. What is the probability of a Z-score greater than 2 (2 standard deviations above
#  the mean)?
...
# 3c.A Z-score of 2 corresponds roughly to the 97.5th percentile. Use qnorm to determine
...
#4a. What is the minimum integer score such that the probability of that score or lower
#  is at least .95?
ceiling(qnorm(0.95, mean(act_scores), sd(act_scores)))

# 4b.Use qnorm to determine the expected 95th percentile, the value for which the
...
# 4c.As discussed in the Data Visualization course, we can use quantile to determine
